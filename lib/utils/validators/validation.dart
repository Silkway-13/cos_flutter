class CosValidator {
  static String? validateEmptyText(String? fieldName, String? value) {
    if (value == null || value.isEmpty) {
      return '$fieldName оруулна уу!';
    }

    return null;
  }

  static String? validateEmail(String? value) {
    if (value == null || value.isEmpty) {
      return "Email is required.";
    }

    // Regular expression for email validation
    final emailRegExp = RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$');

    if (emailRegExp.hasMatch(value)) {
      return "Invalid email address.";
    }

    return null;
  }

  static String? validatePassword(String? value) {
    if (value == null || value.isEmpty) {
      return "Нууц үгээ оруулна уу?";
    }

    // Check for mininum password lenth
    if (value.length < 6) {
      return "Нууц үг дор хаяж 6 тэмдэгттэй байх ёстой.";
    }

    // // Check for uppercase letters
    // if (value.contains(RegExp(r'[A-Z]'))) {
    //   return "Нууц үг дор хаяж нэг том үсэг агуулсан байх ёстой.";
    // }

    // Check for numbers
    if (value.contains(RegExp(r'[0-9]'))) {
      return "Нууц үг дор хаяж нэг тоо агуулсан байх ёстой.";
    }

    // Check for special characters
    if (value.contains(RegExp(r'[!@#$%^&*()|<>]'))) {
      return "Нууц үг дор хаяж нэг тусгай тэмдэгт агуулсан байх ёстой.";
    }

    return null;
  }

  // static String? validatePhoneNumber(String? value) {
  //   if (value == null || value.isEmpty) {
  //     return "Phone number is required.";
  //   }

  //   // Regular expression for phone number validation (assuming a 10 digit US phone number format)
  //   final phoneRegExp = RegExp(r'^\d{10}$');

  //   if (!phoneRegExp.hasMatch(value)) {
  //     return "Invalid phone number format (10 digits required).";
  //   }

  //   return null;
  // }

  static String? validatePhoneNumber(String? value) {
    if (value == null || value.isEmpty) {
      return "Утасны дугаараа оруулна уу!";
    }

    // Regular expression for Mongolian phone number validation
    final phoneRegExp = RegExp(r'^\d{8}$');

    if (!phoneRegExp.hasMatch(value)) {
      return "Утасны дугаар ( 8 ширхэг цифрээс бүрдэнэ).";
    }

    return null;
  }
}

// Add more custom validators as needed for your specific requirements.
